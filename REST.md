# Nexus Marketplace REST API Standards

**The Nexus API Market place** aims to provide better public services for businesses by enabling data and transactions to be exchanged securely and seamlessly through APIs. Nexus aims to provide a product that enables businesses to discover government data sources and existing data APIs, and ultimately to build innovative digital solutions from gaining access to these APIs.
...
Have an obvious mechanism for clients to report issues and ask questions about the API. Use public forums and social media where possible to ensure a large audience can benefit from any questions and resolutions.
...
Common ways to do this include a mailing list, forums, or a [dedicated developer blog](https://developer.github.com/changes/) with an RSS feed.
...
* The URL query string (e.g. `?year=2015`)
...
For just dates, that looks like `2015-02-27`. For full times, that's of the form `2015-02-27T10:00:00Z`.
...
Always require API consumers to authenticate with an API key, in order to get a better understanding of who is using the API and how it is being used. It is also important to know the API users in case you need to make changes to the API or decommision it.

Make it easy for users to obtain an API key, particularly if the API is purely for dissemintion of low risk read only data. Acquiring an API key needs to be self service driven and allow users to instantly generate the required key, unless there are valid reasons for a more thorough sign up process.
...
*These API standards are based on [18F's](https://18f.gsa.gov/) [API standards](https://github.com/18F/api-standards)*
